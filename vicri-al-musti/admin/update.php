...
<head>
    ...
    <title>Edit Data - Premium Alumni Core</title>
...
<body id="body-layout" class="bg-gradient-to-br from-slate-50 via-slate-100 to-indigo-50/50 text-slate-800 min-h-full font-sans transition-all duration-500 flex flex-col justify-center items-center p-4">
    ...
    <main class="w-full max-w-xl">
        ...
        <div class="bg-white p-8 rounded-3xl shadow-xl border border-slate-200/60 relative overflow-hidden transition-all">
            <h2 class="text-2xl font-black text-slate-900 tracking-tight mb-6">Perbarui Data Alumni</h2>
            
            <form action="" method="post" class="flex flex-col gap-5">
                <div class="flex flex-col gap-1.5">
                    <label class="text-xs font-bold text-slate-500 uppercase tracking-wider pl-0.5">Nama Lengkap</label>
                    <input type="text" name="nama" value="<?= htmlspecialchars($data['nama_lengkap']) ?>" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-900 focus:outline-none focus:border-slate-900 focus:bg-white transition-all">
                </div>

                <div class="flex flex-col gap-1.5">
                    <label class="text-xs font-bold text-slate-500 uppercase tracking-wider pl-0.5">Tahun Lulus</label>
                    <input type="number" name="angkatan" value="<?= htmlspecialchars($data['angkatan']) ?>" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-900 focus:outline-none focus:border-slate-900 focus:bg-white transition-all">
                </div>
                ...
                <div class="mt-2">
                    <button type="submit" name="update" class="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold text-sm py-3.5 px-4 rounded-xl shadow-md transition-all cursor-pointer">
                        Perbarui Data
                    </button>
                </div>
            </form>
